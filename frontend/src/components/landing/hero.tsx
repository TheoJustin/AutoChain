...
function Hero() {
    ...
    const handleScroll = () => {
      if (sectionRef.current) {
        ...
        const progress = Math.max(
          0,
          Math.min(1, -sectionTop / (sectionHeight * 0.5))
        );
        setScrollY(progress);
      }
    };
  ...
  const textTransform = isLargeScreen
    ? `translateX(${-scrollY * 100}px) translateY(${scrollY * 50}px)`
    : "none";
  const imageTransform = isLargeScreen
    ? `translateX(${-scrollY * 800}px) scale(${1 + scrollY * 0.1})`
    : "none";
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <p className="text-xl text-gray-600 mb-8 leading-relaxed">
              Experience the future of car rental with AutoChain. Each vehicle
              is minted as a unique CarNFT (ERC-721) with smart contract
              automation. Our AI provides personalized recommendations while
              blockchain technology ensures secure, trustless transactions and
